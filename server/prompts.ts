...
15. Каждый threatProfile подтверждён реальными способностями и ролью NPC. dangerous и выше имеет полный powerBasis/combatIdentity, буквальные signatureAbilities, разные threatVectors, защиту и условия поражения; elite и выше меняет доктрину через engagementPhases; legendary/mythic дополнительно имеет подтверждённые feats, несколько слоёв защиты, подготовленные ресурсы, информационные преимущества, риск для окружения и mastery нужного уровня. Ни одна высокая метка не существует без фактической механики.
