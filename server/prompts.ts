...
Не используй null. Не создавай значения-заглушки, не удаляй данные без прямой просьбы, не меняй числовые показатели случайно. Изменение предмета, NPC, способности или артефакта всегда ссылается на точный id. Полную потерю предмета выражай inventory remove. Класс предмета сверяй со всеми полями rarityProfile; число экземпляров влияет только на дефицит и никогда в одиночку не даёт legendary.
