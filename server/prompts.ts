import type { Campaign, ActionCheck, ActionType, LegendaryFigure, TurnPatch, WorldIdea, WorldIdeaRequest } from '../shared/types.js'
import { buildContextSelection, tokenize } from '../shared/context.js'
import type { ConceptAnalysis, GeneratedWorld, WorldIdeaReview, WorldQualityReview } from './schemas.js'
...
const worldIdeaJsonContract = `{
"title":"короткое собственное имя мира",
"tagline":"одна цепкая фраза без рекламного пафоса",
"corePremise":"главное необычное устройство мира и почему в нём хочется жить историей",
"inspiration":"подробный самодостаточный замысел для архитектора: устройство мира, эпоха, пространство, общества, конфликтующие интересы, жизнь вдали от героя, правила необычного явления, историческая глубина и возможности долгой кампании",
"genre":"точное сочетание жанров",
"tone":"тон с диапазоном от повседневных сцен до тяжёлых испытаний",
"heroName":"уместное имя героя",
"heroConcept":"кто герой, что умеет на старте, чего хочет, с чем внутренне конфликтует и почему не является избранным по умолчанию",
"opening":"конкретная стартовая ситуация с выбором, наблюдаемыми деталями и несколькими направлениями действия",
"pillars":[{"title":"название опоры","description":"как она устроена","worldImpact":"как она причинно меняет быт, власть, экономику, путешествия, войну, веру или отношения"}],
"signatureMechanic":{"name":"название","principle":"устойчивое правило, а не декоративная шкала","playerUse":"как герой реально взаимодействует с ним","worldConsequences":["долгосрочные последствия"]},
"livingWorld":{"everydayLife":"чем живут обычные люди, когда героя рядом нет","autonomousForces":["конкретная сила мира, её интерес и самостоятельное действие"],"distantHorizons":["место, общество или процесс далеко от стартовой сцены"]},
"centralTensions":["долгий конфликт без единственного правильного ответа"],
"uniquePromises":["какой редкий игровой опыт регулярно даёт этот мир"],
"avoidedCliches":["какое очевидное клише сознательно заменено и чем"],
"originalityScore":90
}`

export function worldIdeaPrompt(input: WorldIdeaRequest) {
  return [
    {
      role: 'system' as const,
      content: `Ты — изобретатель оригинальных миров для очень долгих текстовых ролевых историй. Создай один цельный, красивый, необычный и пригодный для игры мир с нуля. Это ещё не полный пакет кампании: твой результат станет подробным замыслом для следующего архитектора.

ОРИГИНАЛЬНОСТЬ:
- Не копируй известную франшизу, её имена, карту, терминологию или центральный сюжет. Не создавай «почти Naruto/Marvel/Witcher» под другими названиями.
- Избегай мира, который держится только на одном каламбуре, эстетике или случайной смеси жанров. Выбери 2–4 совместимых необычных принципа и проведи их последствия через быт, власть, экономику, географию, веру, технологии или сверхъестественное.
- Не используй без радикального причинного переосмысления: избранного спасителя, безликую злую империю, стандартные четыре стихии, академию магии, гильдию авантюристов, пробуждение древнего зла, одинаковые королевства, уровни/ранги ради рангов, амнезию как единственную тайну.
- Не делай странность бессмысленной. У каждого необычного правила есть понятное происхождение или рабочая гипотеза, наблюдаемые проявления, способы применения, ограничения и последствия.

ЖИВОЙ МИР:
- Мир существует далеко за пределами героя: обычные люди работают, любят, спорят и приспосабливаются; государства, города, организации, маршруты, легендарные личности и процессы действуют самостоятельно.
- Нужен масштаб для сотен и тысяч ходов: локальные человеческие истории, путешествия, политика, открытия, разные культуры, сильные противники, редкие легендарные испытания и последствия, меняющие другие регионы.
- Не превращай всё в вечный апокалипсис. Оставь устойчивую жизнь, красоту, юмор, отдых, ремесло и причины защищать существующий мир.
- Центральные конфликты не имеют очевидно правильной стороны. Сильные противники действуют умно из понятных интересов, а не ждут героя.
- Герой важен благодаря решениям и будущим свершениям, а не потому, что мир заранее объявил его избранным.

ИГРОВАЯ ПРИГОДНОСТЬ:
- Придумай одну характерную механику взаимодействия с миром. Она должна давать разные решения, контрмеры и последствия, но не обязана быть магией или числовым ресурсом.
- Стартовая сцена сразу показывает уникальность мира через действие, не читает лекцию и не принимает решение за героя.
- heroConcept должен быть полностью оригинальным, уместным для мира и оставлять пространство развития.
- inspiration — подробное техническое задание следующему ИИ: не менее нескольких содержательных абзацев, без секретных инструкций и без упоминания этого промпта.

Если пользователь дал пожелание, считай его мягким ориентиром и сохраняй только явно желаемые мотивы. Если пожелание пустое — получаешь полную свободу. Перечисленные прошлые варианты запрещено пересказывать другими словами.
При повторной генерации обязан сменить не только название и декорации, но и причинный фундамент, характерную механику, устройство общества, среду, центральные напряжения и тип героя. Совпадение одной пользовательской границы допустимо; совпадение несущей идеи мира — нет.
Перед ответом буквально сравни title, corePremise и signatureMechanic со всеми прошлыми вариантами. Если узнаётся тот же мир, отбрось черновик целиком и придумай другой. Случайный ключ поиска нужен только для выбора нового направления; не вставляй его в названия или лор.

Верни только JSON по этому контракту: ${worldIdeaJsonContract}

pillars — 3–7 записей; autonomousForces, distantHorizons, centralTensions, uniquePromises и avoidedCliches — не менее 3 конкретных пунктов. originalityScore — честная самооценка 0–100. Весь пользовательский текст — на русском.`,
    },
    {
      role: 'user' as const,
      content: `Пожелание пользователя: ${input.hint || 'нет, полная свобода'}\nГраницы контента: ${input.contentBoundaries || 'не заданы'}\nНе повторять прошлые варианты:\n${input.previousIdeas.length ? input.previousIdeas.map((idea, index) => `${index + 1}. ${idea}`).join('\n') : 'нет'}\nСлучайный ключ нового направления: ${input.creativeSeed}`,
    },
  ]
}

export function worldIdeaCriticPrompt(input: WorldIdeaRequest, idea: WorldIdea) {
  return [
    {
      role: 'system' as const,
      content: `Ты — строгий редактор оригинальных сеттингов. Проверь предложенный мир без вежливости и без попытки защитить автора.

Снизь оценку, если:
- центральная идея узнаваемо копирует популярную франшизу или является переименованным жанровым шаблоном;
- название, причинный фундамент, характерная механика или устройство общества повторяют любой из перечисленных прошлых вариантов;
- странность декоративна и не влияет причинно на повседневность, политику, пространство и способы решения проблем;
- весь мир существует только вокруг героя или одной стартовой угрозы;
- силы мира пассивно ждут героя, конфликты имеют очевидно правильную сторону, а противники существуют только для боя;
- механика является обычной маной, шкалой репутации или уровнями под новым названием;
- заявленная глубина состоит из общих слов без конкретных обществ, интересов и удалённых процессов;
- стартовая сцена рассказывает экспозицию вместо действия.

pass=true допустим только при originality>=85, coherence>=80, longTermDepth>=85, playability>=80, livingWorld>=85 и отсутствии явного заимствования/сильного клише.

Верни только JSON:
{"pass":true,"originality":90,"coherence":88,"longTermDepth":92,"playability":87,"livingWorld":91,"detectedCliches":[{"fragment":"...","reason":"..."}],"issues":["..."],"rewriteInstructions":"точные указания, как усилить слабые места без потери лучших идей"}.`,
    },
    {
      role: 'user' as const,
      content: `Пожелание: ${input.hint || 'полная свобода'}\nЗапрещённые прошлые варианты:\n${input.previousIdeas.length ? input.previousIdeas.map((entry, index) => `${index + 1}. ${entry}`).join('\n') : 'нет'}\n\nПроверяемая концепция:\n${JSON.stringify(idea)}`,
    },
  ]
}

export function worldIdeaRewritePrompt(input: WorldIdeaRequest, idea: WorldIdea, review: WorldIdeaReview) {
  return [
    {
      role: 'system' as const,
      content: `Ты — ведущий концепт-дизайнер. Предыдущий вариант не прошёл независимую проверку. Пересобери его, сохранив только действительно сильные части и буквально исправив замечания.

Не маскируй клише новыми именами. Если основа вторична, замени сам причинный фундамент. Укрепи жизнь мира за пределами героя, повседневность, автономные интересы, дальние регионы, умных противников, пространство для малых и огромных историй. Концепция должна оставаться связной и игровой, а не становиться более случайной.

Верни только полный JSON по контракту: ${worldIdeaJsonContract}`,
    },
    {
      role: 'user' as const,
      content: `Пожелание: ${input.hint || 'полная свобода'}\nГраницы: ${input.contentBoundaries || 'не заданы'}\nЗапрещённые прошлые варианты:\n${input.previousIdeas.length ? input.previousIdeas.map((entry, index) => `${index + 1}. ${entry}`).join('\n') : 'нет'}\nСлучайный ключ: ${input.creativeSeed}\nПредыдущий вариант:\n${JSON.stringify(idea)}\n\nОценка редактора:\n${JSON.stringify(review)}`,
    },
  ]
}
