import type { Campaign, ActionCheck, ActionType, EventDirectorState, LegendaryFigure, NarrativeEventDecision, TurnPatch, WorkshopEventOptions, WorldNoveltyReference, WorldQuestionMessage, WorldQuestionScope } from '../shared/types.js'
import { buildContextSelection, tokenize, type ContextCaps } from '../shared/context.js'
...
import { analyzeWorldRequestIntent, worldIntentPrompt } from './world-intent.js'
...
- ability.profile обязателен и содержит nature, creativeIdentity, ownerExpression, standing, facets, presentation, discovery, availability только при реальном ограничении и developmentSeeds. nature.kind — только техническая природа innate|trained|technological|social|authority|access|economic|organizational|contractual|divine|psionic|magical|biological|other, а не название школы вроде ninjutsu или kekkei-genkai. nature.groupId и standing.tierId обязательны, никогда не пропускаются и ссылаются только на точные id из capabilitySystem; standing.systemId — точный id самой системы. facets — только массив из 2–6 объектов {key,label,value,description}, никогда не словарь label:value. availability содержит только state, обязательный reasons (пустой массив допустим), nextReady, charges и lastUsedTurn. charges добавляй исключительно для реально существующей механики конечных зарядов и только в форме {current:целое_от_0,max:целое_от_1,label:"точное название зарядов"}; если зарядов у способности нет, полностью пропусти charges — никогда не возвращай max:0 и не придумывай заряд ради заполнения поля. nextReady имеет точную форму {unit:'turn'|'scene'|'day'|'condition', value?:number, condition?:string}; не возвращай поля turn, scene или day вместо value. costs, cooldown, requirements и limitations принадлежат самой ability, а недостатки силы записываются в ability.limitations. presentation.sectionOrder, discovery.revealedSections и discovery.evidence[].section используют только identity|principle|source|standing|facets|availability|techniques|counterplay|progression|history; не возвращай nature, creativeIdentity или ownerExpression.
...
- availability, costs, cooldown, заряды, блокировки, требования и слабости существуют только если следуют из природы силы, канона или уже установленного факта. Это независимые механики, а не обязательный набор: способность может быть полностью бесплатной, без зарядов и без отката; другая может расходовать существующий числовой ресурс; третья — зависеть только от условия, времени, риска, позиции, инструмента или объективного последствия. Не создавай mana/energy/focus/charges только ради способности. Нечисловое последствие отражай настоящим statePatch (статус, время, предмет, отношение и т. п.), а не выдуманным costs. availability.state использует только ready|limited|cooldown|blocked|disabled; не возвращай available. Никогда не придумывай плату «для баланса». Пустые или пропущенные costs/limitations/requirements означают их фактическое отсутствие.
...
- При использовании конкретной подспособности учитывай именно её unlocked, mastery, effects и все реально существующие costs/requirements/limitations; списывай цену только при непустом costs и обновляй её собственное mastery, не ограничиваясь общим mastery родительской силы.
...
- Новая сила требует ability/create со стабильным targetId либо ability/update существующей записи и реального addAbilities/abilityChanges. При создании полностью опиши источник, тип, активацию, возможности, эффекты, уместные техники, синергии, контрмеры, примеры и канонический статус. Ограничения, требования, costs, cooldown, заряды, развитие и ветви эволюции добавляй только когда они действительно существуют; пустой массив или отсутствие поля не является неполнотой.
...
- world.upsertLegends содержит ТОЛЬКО отдельных персонажей: людей, разумных существ или устойчивые личностные сущности. Катастрофа, война, пророчество, место, артефакт, организация, культ, эпоха или природное явление не являются legendaryFigure и должны обновляться через worldEvents, processes, lore, mysteries, factions, places, mechanics либо артефакты.
- name — имя самого персонажа или его устойчивый личный идентификатор, а не название подвига или главы биографии. «Аурел Семипечатный» и «Последний Эфирный Мастер» допустимы; «Падение Аурела», «Восхождение Короля», «Пророчество о герое» и «Война Кристаллов» — нет. Падение, восхождение, рождение, войну и иной эпизод помещай в deeds/myths. При наличии characterId name буквально совпадает с именем связанного героя/NPC; прозвища и титулы лежат в aliases/titles/epithet.
...
- Герой не является вложенным объектом statePatch. Никогда не возвращай statePatch.player/statePatch.hero. Профиль меняй через playerProfile; абсолютные характеристики через upsertStats, ресурсы через upsertResources, валюту через upsertCurrency; добавочные изменения — через statDeltas/resourceDeltas/currencyDeltas; способности — через addAbilities/abilityChanges/removeAbilityIds; состояния — через addConditions/upsertStatusEffects и соответствующие remove-поля.
...
function promptContextCaps(campaign: Campaign, audience: PromptAudience): ContextCaps | undefined {
  const mode = campaign.settings.qualityMode ?? 'balanced'
  if (mode === 'deep') return undefined
  if (mode === 'fast') return audience === 'narrative'
    ? { budgetChars: 120_000, recentMessages: 24, lore: 10, memories: 14, documents: 6, archives: 16 }
    : audience === 'background'
      ? { budgetChars: 180_000, recentMessages: 36, lore: 14, memories: 20, documents: 8, archives: 24 }
      : { budgetChars: 160_000, recentMessages: 32, lore: 12, memories: 18, documents: 8, archives: 20 }
  return audience === 'narrative'
    ? { budgetChars: 220_000, recentMessages: 36, lore: 14, memories: 22, documents: 8, archives: 28 }
    : audience === 'background'
      ? { budgetChars: 360_000, recentMessages: 64, lore: 22, memories: 34, documents: 16, archives: 44 }
      : { budgetChars: 320_000, recentMessages: 52, lore: 18, memories: 30, documents: 14, archives: 36 }
}

function compactCampaign(campaign: Campaign, input: string, audience: PromptAudience = 'story') {
  const selected = buildContextSelection(campaign, input, promptContextCaps(campaign, audience))
  ...
  const worldLegends = selectFocused(
    (campaign.world.legends ?? []).filter(legendRepresentsCharacter),
    queryTokens,
    (legend) => [
      legend.name,
      legend.aliases.join(' '),
  ...
  const legendIndex = (campaign.world.legends ?? [])
    .filter(legendRepresentsCharacter)
...
function campaignEditorContext(campaign: Campaign, input: string) {
  const selected = buildContextSelection(campaign, input, promptContextCaps(campaign, 'background'))
...
- Способности — строгие, но индивидуальные игровые сущности. При применении учитывай только реально заданные kind, mastery, costs, effects, limitations, requirements, cooldown, availability, charges и evolutionPaths. Списывай ресурс лишь когда непустой costs прямо задан способности или технике; отсутствие цены означает бесплатное применение и не требует создавать ману, энергию либо иной ресурс. После фактически сработавшей способности abilityChanges обязателен: при известном итоговом уровне запиши абсолютный mastery, а при росте от практики — masteryDelta обычно 1–5; не возвращай оба. Добавь history по канонической форме ниже; при блокировке или неудачной попытке не выдавай рост мастерства, но можешь записать содержательный history. Не дописывай старой способности отсутствующие costs, cooldown, requirements, limitations, progression, tags, заряды или ветви развития только для заполнения карточки. Новая способность обязана иметь полную собственную механику, но цена, ограничение, откат и эволюция у неё могут объективно отсутствовать.
- kind=active/reaction/ritual/transformation не срабатывает просто потому, что мог бы помочь автору: требуется заявленная активация, заранее установленный автоматический trigger либо явное решение владельца-NPC. Выполняй requirements и оплачивай costs только если они существуют. kind=passive действует постоянно только в границах собственных effects. Если «Чувство лжи» не сработало или не было активировано, запрещено обойти это фразой вроде «герой всё равно ощущает ложь кожей», интуицией без механики или авторским знанием. Перенести эффект способности в обычную прозу — такое же нарушение контракта, как выдумать новую силу.
...
- Способности NPC имеют ту же глубину, что способности героя, но не один шаблон: description, source, kind, mastery, effects, category, scale, activation, capabilities, synergies, counters, examples и canonStatus раскрывают сущность силы; rank, costs, limitations, requirements, progression, evolutionPaths, history, tags, cooldown и заряды присутствуют только когда уместны. При проявлении ранее неописанной силы существующего NPC добавь её полной записью через npc.upsertAbilities, не своди к названию и одному эффекту. Непустую установленную цену NPC списывай через npc.resourceDeltas; бесплатная сила ничего не списывает. Последствия отражай через настоящие npc.status/stat/resource поля, развитие и историю — через npc.abilityChanges с точным abilityId.
...
- Если во вводе назван класс common|uncommon|rare|exceptional|epic|legendary|mythic|transcendent или его русское название и в этом же ходе запрос действительно исполнен через inventory add, итоговая вычисляемая редкость должна быть не ниже запрошенной. Transcendent — высший класс над mythic, а не его синоним: для него нужны potency>=95, worldImpact>=98, max(versatility,provenance)>=85, max(дефицит,acquisitionRisk)>=80 и реальная власть над фундаментальным законом. Mythic требует potency>=88, worldImpact>=91 и max(versatility,provenance)>=75, но использует законы мира, а transcendent способен их переписывать. Не снижай класс за честные требования, но и не завышай оценки слабому предмету.
...
ШКАЛА СИЛЫ, КОТОРУЮ НЕЛЬЗЯ ПУТАТЬ:
- legendary — исключительный предмет, способный решать битвы и влиять на регионы, но остающийся внутри обычных законов мира;
- mythic — эпохальная сила глобального или космического значения, способная перестраивать системы мира, но всё ещё использующая его фундаментальные законы;
- transcendent — высший класс над mythic: собственная власть над фундаментальным законом, способная конкретно переписать, отменить, создать или навязать предел реальности. Это качественно иной уровень, а не просто больше урона.
Фразы «максимальная», «высшая», «наивысшая» или «самая сильная редкость» всегда означают transcendent. Мировое красивое название класса не меняет машинный порядок legendary < mythic < transcendent.
...
- Способности показывай через конкретное действие, ощущение и наблюдаемый эффект. Цену, ограничение, cooldown, заряды или требования показывай только если они действительно записаны в утверждённой механике; их отсутствие не нужно маскировать новой платой. Не добавляй силу, которой нет в плане, и учитывай уровень mastery.
- Активная, реактивная, ритуальная или трансформационная способность требует показанной активации/trigger. Выполненные requirements и оплаченную цену показывай только при их наличии в плане. Пассивная действует только в границах effects. Если проверка силы не дала результата, не выдавай тот же результат «обычной интуицией», кожным чувством, сном, эхом или авторским намёком.
- Если применяется именованная подспособность из techniques, показывай именно её механику и масштаб, а также только её реально существующую цену/условия. Не приписывай одному приёму возможности соседних техник только потому, что они принадлежат общей родительской силе.
...
export function continuityCriticPrompt(
  campaign: Campaign,
  input: string,
  actionType: ActionType,
  plan: unknown,
  draftA: string,
  draftB: string,
  repetitionA: NarrativeRepetitionIssue[] = [],
  repetitionB: NarrativeRepetitionIssue[] = [],
) {
  const observedPlan = narrativePlanView(plan, campaign)
  const context = compactCampaign(campaign, `${input}\n${JSON.stringify(observedPlan)}\n${draftA}\n${draftB}`, 'narrative')
  return [
...
Сверь оба варианта с context.narrativeFingerprint и ПРОГРАММНОЙ ПРОВЕРКОЙ ПОВТОРОВ. Предпочти черновик без программных нарушений. Нарушение high всегда требует pass=false и точного переписывания отмеченного абзаца, даже если остальная сцена хороша. Не повторяй недавнее начало/окончание, repeatedParagraphs/repeatedPhrases и не строй сцену из привычных «на мгновение», взглядов, дрогнувших рук, грома/мигания света и финала, где все смотрят и ждут. Отклоняй хоровую реакцию NPC, авторские объяснения скрытых мотивов вместо наблюдаемых действий, вечные незавершённые жесты и отсутствие материального/социального/информационного итога заявленного действия. Предпочти вариант, где NPC продолжают собственные цели, а обширный мир виден через причинно дошедший сигнал, не через энциклопедию или случайную угрозу герою.
...
Отклоняй вариант, если он без причинного триггера вводит легендарного/мифического врага или вмешательство бога, делает неизбежное смертельное испытание без предупреждения и доступного выхода, обесценивает установленную исключительную силу лёгкой победой, заставляет организацию знать тайное событие без канала информации либо превращает respite/exploration в очередное нападение вопреки плану. Сильный NPC не забывает signatureAbilities, defensiveLayers, preparedAssets и доступную engagementPhase; его поражение допустимо только из actionCheck, constraints, defeatRequirements или реально созданного преимущества. При этом не превращай силу во всеведение или сюжетную неуязвимость. Если несколько предыдущих сцен уже были напряжёнными, проверь, что новая эскалация действительно неизбежна, а не создана по привычке. Выбери a или b. pass=true только если выбранный вариант не требует смыслового исправления. Если есть проблема, дай конкретные rewriteInstructions. Верни только JSON строго вида {"chosen":"a","pass":true,"issues":[{"type":"continuity","detail":"...","severity":"medium"}],"rewriteInstructions":"..."}. chosen — только a или b; pass — только boolean; type — canon|continuity|knowledge|agency|state|style; severity — low|medium|high. Если проблем нет, issues должен быть пустым массивом, а rewriteInstructions — пустой строкой.`,
    ...
    { role: 'user' as const, content: `КОНТЕКСТ:\n${JSON.stringify(context)}\n\nВВОД (actionType=${actionType}; ${actionLabels[actionType]}):\n${input}\n\nНАБЛЮДАЕМАЯ ЧАСТЬ ПЛАНА:\n${JSON.stringify(observedPlan)}\n\nПРОГРАММНАЯ ПРОВЕРКА ПОВТОРОВ A:\n${JSON.stringify(repetitionA)}\n\nПРОГРАММНАЯ ПРОВЕРКА ПОВТОРОВ B:\n${JSON.stringify(repetitionB)}\n\nЧЕРНОВИК A:\n${draftA}\n\nЧЕРНОВИК B:\n${draftB}` },
  ]
}

export function narrativeRepetitionRevisionPrompt(
  campaign: Campaign,
  input: string,
  plan: unknown,
  draft: string,
  ...
  return [
    {
      role: 'system' as const,
      content: `Ты — точный редактор против самоповторов в долгой текстовой RPG. Верни всю сцену целиком на русском языке, но перепиши только отмеченные программной проверкой абзацы и необходимые переходы.

Обязательные правила:
- Сохрани без изменений все факты, причинный порядок, результаты действий, числа, реплики пользователя, раскрытую информацию и последствия утверждённого плана.
- Не добавляй герою новую реплику, мысль, чувство, взгляд, жест, движение или решение.
- Не заменяй повтор синонимами и перестановкой тех же образов. Если описание среды не сообщает нового факта, удали его. Если сообщает — покажи другой конкретный аспект, который влияет на действие сейчас.
- В одном ответе нельзя дважды пересказывать одинаковый вид из окна, тишину, свет, гул, дыхание, ожидание или расположение предметов без произошедшего изменения.
- Не сокращай содержательные действия NPC и не создавай новых событий сверх плана. Верни только готовую художественную сцену без пояснений редактора.`,
    },
    {
      role: 'user' as const,
      content: `ВВОД ИГРОКА:\n${input}\n\nНАБЛЮДАЕМАЯ ЧАСТЬ ПЛАНА:\n${JSON.stringify(observedPlan)}\n\nТОЧНЫЕ НАРУШЕНИЯ ПОВТОРОВ:\n${JSON.stringify(issues)}\n\nДРУГИЕ ОБЯЗАТЕЛЬНЫЕ ИСПРАВЛЕНИЯ:\n${additionalInstructions || 'Нет'}\n\nИСХОДНАЯ СЦЕНА:\n${draft}`,
    },
  ]
}
...
4.1. Проверь сам факт активации: active/reaction/ritual/transformation требует заявленной активации, установленного trigger либо явного решения NPC. requirements и costs проверяй только когда они реально заданы; бесплатная способность без условий не получает выдуманную цену. Если способность молчала/провалилась, narrativePass=false, когда проза всё равно выдаёт тот же результат «интуицией», ощущением или авторским намёком.
...
const worldCreativeLenses = [
  'Выведи устройство поселений и маршрутов из конкретной физической среды, а не из жанровой декорации.',
  'Придумай обычный труд, обмен, дефицит и бытовую привычку, которые меняют решения людей каждый день.',
  'Создай институт, который решает реальную общественную задачу необычным для жанра способом.',
  'Заложи историческое решение, которое когда-то было разумным, но теперь создаёт новое противоречие.',
  'Сделай границы регионов следствием логистики, права, экологии или времени, а не просто линиями на карте.',
  'Определи, кто хранит знания, кто имеет право им не верить и как ошибка становится общественным фактом.',
  'Создай независимую от героя цепочку производства, обслуживания или выживания с несколькими участниками.',
  'Дай миру форму досуга, праздника, траура или состязания, которая раскрывает его ценности без экспозиции.',
  'Построй конфликт между двумя рациональными моделями будущего, а не между светом и абстрактным злом.',
  'Свяжи повседневную архитектуру с климатом, телами жителей, транспортом и формой собственности.',
  'Покажи, как далёкий регион влияет на стартовый город через цены, миграцию, право, моду или слухи.',
  'Создай профессию и инфраструктуру, без которых мир перестанет работать, хотя герой к ним не относится.',
  'Определи, что общество считает доказательством, честной сделкой и законным насилием.',
  'Сделай одну важную технологию или практику массовой и скучной для жителей, но удивительной для читателя.',
  'Добавь медленную перемену эпохи, которую никто не контролирует полностью и герой не запускал.',
  'Пусть разные культуры используют один закон природы по несовместимым, но логичным правилам.',
] as const

function creativeLensSelection(seed: string): string[] {
  let hash = 2166136261
  for (const char of seed) hash = Math.imul(hash ^ char.codePointAt(0)!, 16777619) >>> 0
  const start = hash % worldCreativeLenses.length
  // The list has 16 entries; every odd step is coprime with 16, so the first five
  // indices are guaranteed to be distinct and this selection can never loop.
  const step = [1, 3, 5, 7, 9, 11, 13, 15][(hash >>> 8) % 8]
  return Array.from({ length: 5 }, (_, index) => worldCreativeLenses[(start + index * step) % worldCreativeLenses.length])
}

function worldOriginalityDirective(input: WorldConceptInput): string {
  const seed = input.creativeSeed || `${input.inspiration}|${input.characterConcept}|${input.opening}`
  const requestIntent = analyzeWorldRequestIntent(input)
  const references = (input.noveltyReferences ?? []).map((entry) => ({
    name: entry.name,
    tagline: entry.tagline,
    premise: entry.premise,
    signatureTerms: entry.signatureTerms,
  }))
  return `
${worldIntentPrompt(input)}

НЕЗАВИСИМАЯ АВТОРСКАЯ ОСНОВА МИРА:
- Применяй тест замены героя: фундаментальное описание, география, история, институты, экономика, законы, большинство фракций и автономных процессов должны оставаться осмысленными, если заменить героя другим человеком или убрать его из истории. Герой влияет на будущее, но не является тайной причиной существования всего сеттинга, если пользователь прямо этого не попросил.
- Сначала придумай причинный мир и жизнь общества, затем помести в него героя. Система возможностей описывает всё множество путей жителей; стартовые способности героя занимают лишь подходящее подмножество и не определяют названия всех групп, ресурсов, законов и эпох.
- Не называй мир по универсальной субстанции или способности героя без прямого требования пользователя. Не используй по умолчанию эфир/эфириум, резонанс, разлом, осколки, кристаллы, эхо, пустоту, безымянную древнюю энергию, избранного и пророчество. Если такой мотив прямо задан, разработай его конкретно и не делай объяснением абсолютно всего.
- Уникальность означает конкретных людей, культуры, причины и конфликты, а не обязательно экспериментальную основу. Знакомый жанровый каркас можно сохранить, если его просит пользователь; одни лишь новые синонимы всё так же не считаются уникальностью.
- Большинство крупных сил и процессов уже имеют собственные цели до встречи с героем. Не связывай всех NPC, легенды, кризисы и секреты с его происхождением.
Творческие вопросы именно этой генерации (это углы исследования, а не готовый шаблон):
${creativeLensSelection(seed).map((lens) => `- ${lens}`).join('\n')}
${references.length ? `Уже существующие миры пользователя, которые нельзя повторять, переименовывать или механически смешивать:\n${JSON.stringify(references)}\n${requestIntent.mode === 'familiar' || requestIntent.mode === 'guided' ? 'Одинаковые жанровые опоры не считай повтором: сравнивай только отличительные причины, культуры, конфликты, географию и собственные термины.' : ''}` : 'Предыдущих миров для сравнения нет; всё равно избегай перечисленных модельных клише.'}
Ключ вариации: ${seed.slice(0, 18)}.`
}
...
const generatedTechniqueShape = 'techniques[{name,description,kind,category,mastery,activation,scale,costs?[{resource,amount}] (только ключ существующего ресурса и числовая величина; без label),effects[],requirements?[],limitations?[],unlocked}]'

export function conceptAnalystPrompt(input: WorldConceptInput) {
  return [
    {
      role: 'system' as const,
      content: `Ты — эксперт по канону вымышленных вселенных и дизайнер самостоятельных миров и систем возможностей. До создания мира разберись, какие именно известные сущности назвал пользователь, какой мир он ожидает и какой опыт силы нужен герою.

${worldOriginalityDirective(input)}
...
Снача буквально определи ожидаемый опыт от всего мира, а не только от силы героя. Если просят обычный, классический или узнаваемый жанр, запиши его ключевые конвенции в originalityRules как то, что нужно сохранить, а не избежать. powerFantasy здесь означает весь желаемый опыт кампании: тип приключения, место героя в мире, жанровую фантазию и уровень обычности/необычности. Не подменяй это перечнем способностей.
...
ПРИНАДЛЕЖНОСТЬ СПОСОБНОСТЕЙ — ЖЁСТКОЕ ПРАВИЛО ДАННЫХ:
- player.abilities содержит только способности самого героя: врождённые, изученные, социальные, технологически встроенные в него или навсегда переданные ему так, что они сохранятся после потери предмета.
- Любая сила, которая существует благодаря находящемуся у героя предмету, записывается только в inventory[].artifact.powers, passiveEffects или combinedEffects. Интерфейс сам покажет её в отдельном разделе «Силы предметов»; не копируй её в player.abilities даже ради удобства, полного досье или выполнения capabilityChecklist.
- Одинаковые name/description/effects в player.abilities и artifact.powers являются ошибкой. Если способность действительно навсегда отделилась от предмета, прямо установи в source и истории необратимый перенос и объясни, почему она продолжает существовать без предмета; простая экипировка, настройка, связь или владение переносом не считаются.
- Потеря, поломка, запечатывание или снятие предмета должна менять доступность его проекции, но не личные способности героя. Личная сила, наоборот, не исчезает вместе с инвентарём.
...
- В каждом мире создай минимум 10 полноценных исключительных фигур; для обширного мира нормально 12–18. Каждая из этих фигур — отдельный ПЕРСОНАЖ. world.legends не является каталогом событий: война, катастрофа, пророчество, место, артефакт, культ, организация или загадка живут в предназначенных для них разделах. Это не десять боссов и не десять вариаций одного архетипа: включи живых современников, пропавших или скрытых носителей силы, исторических деятелей, восходящие имена, легендарных и крайне редких мифических фигур.
...
- Источник знает только перечисленное в knowledge. Если pressure является ответом на поступок героя, cause объясняет канал получения сведений. Не выдавай корпорации, государству, клану или божеству мгновенное всеведение. sourceNpcName буквально совпадает с именем героя/NPC; sourceName фракции или корпорации буквально совпадает с world.factions[].name. Каждый targetNames буквально совпадает с реально созданным именем героя/NPC, фракции, места, легендарной сущности либо названием world.processes; не используй неизвестное прозвище или понятие без собственной постоянной записи.
...
- world.legends — исключительно список отдельных персонажей. Не записывай сюда войны, падения империй, катастрофы, пророчества, тайны городов, храмы, культы, организации, артефакты или природные явления: для них существуют events/processes/lore/mysteries/places/factions/inventory. name называет личность, а не событие вокруг неё; биографические эпизоды находятся в deeds и myths. Если задан characterName, name должен буквально совпасть с ним.
...
Сначала зафиксируй независимые правила и систему возможностей всего общества, и только затем полностью проработай героя, его реальные способности, ресурсы и стартовые предметы/артефакты. Группы capabilitySystem не должны быть перечнем способностей героя. player.abilities содержит ТОЛЬКО силы, сохраняющиеся без предметов; каждая сила стартового предмета существует ТОЛЬКО в inventory[].artifact.powers/passiveEffects/combinedEffects. Не дублируй одну силу в обоих местах. Ничего не сокращай: этот этап владеет окончательными полями player и inventory.`,
...
Опирайся на уже установленные фундамент, эпоху и правила. Создай обширный причинный мир нужного масштаба, иерархию мест, реальные силы общества и маршруты. География, право, экономика и хотя бы несколько общественных интересов должны существовать независимо от героя и его дара. Не создавай процессы, события, NPC, легенды, лор, стартовую сцену или интерфейс. Не повторяй переданные факты в ответе.`,
...
Все legends — отдельные персонажи, а не события, войны, катастрофы, пророчества, места, организации, артефакты или эпохи. name называет самого персонажа; событие помещается в deeds/myths/lore. Все живые/returned фигуры с characterName должны буквально соответствовать герою или полному NPC из установленных фактов, а их name должен буквально равняться characterName; прозвища хранятся в aliases/titles/epithet. Все места и фракции называй точно. Создай полноценную экологию минимум из десяти уникальных персонажей со всеми требованиями основного контракта, различными эпохами, областями силы, подвигами, мифами, наследием, условиями встречи и постепенным раскрытием. Историческая фигура может не иметь characterName; не создавай для неё фиктивного NPC.`,
...
Все id стабильны, коротки и уникальны. Все ссылки используют буквальные имена из этого же паспорта. legends перечисляет только отдельных персонажей, никогда не события, войны, катастрофы, пророчества, места, организации, артефакты или эпохи. legends[].name — имя личности, не название её судьбы или подвига. У легенды с characterName поле name буквально равно characterName; прозвища позднее попадут в aliases/titles. characterName ставь ТОЛЬКО если этот человек уже дословно присутствует в player.name или npcs[].name; у исторической, мёртвой, недоступной либо просто не включённой в активный состав фигуры поле опусти. openingLocationName существует в places, openingNpcNames — в npcs. NPC.locationName ставь только для точного места из places; если местоположение неизвестно или скрыто, полностью опусти поле вместо «Unknown»/«Неизвестно». NPC.factionNames содержит только точные имена из factions и остаётся пустым, если принадлежность неизвестна. parentName ставь только когда сам родитель отдельной записью существует в places; название страны или региона, отсутствующее в places, не записывай как parentName. controllingFactionName ставь только для фракции из factions. Player abilityNames перечисляет только самостоятельные способности героя, сохраняющиеся без инвентаря, а не примеры применения и не powers предметов из player.inventory. Состав NPC и легенд сразу обеспечивает требуемую экологию сильных, скрытых, исторических и восходящих фигур, но не завышает силу вопреки канону. Не пиши полные досье, способности, законы или сцены — их параллельно создадут следующие этапы.`
    },
    {
      role: 'user' as const,
...
export function worldGenerationManifestOriginalityRepairPrompt(
  ...
  return [
    ...worldGenerationManifestPrompt(input, concept),
    { role: 'assistant' as const, content: JSON.stringify(manifest) },
    {
      role: 'user' as const,
      content: `Паспорт структурно корректен, но проверка авторской самостоятельности отклонила его:\n- ${issues.join('\n- ')}\n\nПересобери ПОЛНЫЙ паспорт. Не исправляй проблему простой заменой слов: измени причинную основу, устройство общества, географическую логику, систему возможностей и независимые линии настолько, насколько требуется. Сохрани прямые требования пользователя и точный канон. Overview не должен зависеть от имени героя; capabilityGroups не должны зеркалить его abilityNames. Верни только полный JSON паспорта.`,
    },
  ]
}
...
6. Составной артефакт имеет компоненты и совместные эффекты; полный предмет заметно больше суммы поверхностных названий. Его центральный закон, происхождение и набор powers/techniques уникальны для конкретного мира, а не собраны из повторяемой заготовки «кристалл/эхо/резонанс/барьер». Mythic подтверждён эпохальными возможностями и порогами potency>=88/worldImpact>=91; transcendent является высшим классом над mythic, подтверждён конкретной властью над фундаментальным законом, potency>=95, worldImpact>=98, max(versatility,provenance)>=85 и max(дефицит,acquisitionRisk)>=80. Одна надпись класса не засчитывается.
...
13. Созданные interfaceBlueprint, metrics и interfaceModules выведены из фактической структуры именно этого мира, а не из жанрового шаблона. Если отдельных наблюдаемых систем нет, interfaceModules=[] является правильным результатом. Если blueprint или модули созданы: blueprint содержит ровно все шесть обязательных tabs dashboard/scene/hero/inventory/changes/world с visible=true, существующую defaultTab и осмысленный порядок dashboardSections; каждый модуль имеет содержательные reason/updatePolicy, уместные placement/visual/pinned/density/emphasis и полезные элементы. Каждая live binding ссылается на существующие в этом же JSON key/id/name и доступную герою сущность; known/rumored-модуль никогда не содержит binding, label или fallback скрытой сущности; стартовый модуль не привязан к ещё не существующему activeConflict. Числовой мироспецифичный показатель оформлен как world.metric с честными source/updatePolicy; stateRules числовые и причинные; links не повреждены, hidden-знание не раскрыто, обычные HP/stats/inventory не продублированы без причины.
...
22. Мир проходит тест замены героя: overview, rules, география, культура, экономика, право, большинство фракций, легенд и processes остаются осмысленными без стартового персонажа. Его способности не переименованы в универсальные законы всего сеттинга и capabilitySystem охватывает пути других жителей.
23. Для original/flexible не повторены прошлые noveltyReferences: сравни не только названия, но причинный двигатель, пространственную организацию, институты, повседневность, материальную культуру, центральный конфликт и терминологию. Новые синонимы при той же конструкции считаются повтором.
24. Без прямого запроса не используются как центральная универсальная основа эфир/эфириум, резонанс, разлом, осколки, кристаллы, эхо, пустота, безымянная древняя энергия, избранный или пророчество. Единичная уместная деталь допустима; повторение мотива в имени мира, системе сил, эпохе, фракциях и сюжете является high originality issue.
25. У мира есть конкретная жизнь за пределами приключения: причинные труд, обмен, инфраструктура, институты, дальние связи, бытовые нормы или иные подходящие сеттингу системы. Не требуй всех категорий сразу и не заполняй их шаблонно, но отклоняй мир, состоящий только из боевых сил героя, врагов и квестов вокруг него.
26. Буквальный опыт из ДОГОВОРА ПОНИМАНИЯ ЗАПРОСА и powerFantasy воплощён во всём мире. Если пользователь просил обычную/классическую/узнаваемую жанровую основу, не снижай оценку за знакомые опоры; снижай её за безликую реализацию или за подмену запроса нарочито странной метафизикой.
27. Для аниме/исекай-запроса мир сначала даёт узнаваемую фантазию жизни и приключения, а затем конкретную глубину. Королевства, гильдии, магия, монстры, народы, ранги и путешествие допустимы и не являются ошибкой сами по себе. Названное произведение в flexible/original задаёт жанровый DNA, но не свои имена и сюжет.
28. player.abilities и силы предметов разделены по владельцу. Сила, доступная только благодаря inventory[].artifact, существует только в powers/passiveEffects/combinedEffects этого предмета и не продублирована среди личных способностей героя. Исключение допустимо лишь при прямо установленном необратимом переносе, после которого личная сила сохраняется без предмета.
...
- affectedDomains обязан содержать каждый уникальный domain из immediateEffects и persistentEffects. Если новая сущность указана в sourceIds, causeIds, scopeIds или participantIds, создай её через requirement operation=create с точно таким же стабильным targetId; одно имя или новый ID без требования создания недопустимы.
...
Не используй null. Не создавай значения-заглушки, не удаляй данные без прямой просьбы, не меняй числовые показатели случайно. Изменение предмета, NPC, способности или артефакта всегда ссылается на точный id. Полную потерю предмета выражай inventory remove. Класс предмета сверяй со всеми полями rarityProfile; число экземпляров влияет только на дефицит и никогда в одиночку не даёт legendary. Фраза «максимальная/высшая редкость» обязательно означает machine rarity=transcendent, а не legendary или mythic; порядок всегда legendary < mythic < transcendent. Если владелец просит mythic/transcendent или исправляет слишком слабый артефакт, перестрой его настоящие description/effects/rarityProfile/artifact.powers/components/passiveEffects/combinedEffects/counters/failureModes согласованно, а не меняй одну метку rarity. Transcendent — высший класс над mythic и требует potency>=95, worldImpact>=98, max(versatility,provenance)>=85, max(дефицит,acquisitionRisk)>=80 и конкретной власти над фундаментальным пределом мира. Новые силы предмета не копируй в addAbilities: вкладка героя получает их напрямую из inventory.artifact.powers; addAbilities нужен только для постоянной личной силы, существующей без предмета. Существующий артефакт полностью перерабатывай только по явному запросу владельца; обычное повышение mastery не меняет creativeIdentity.
