...
- Изменение NPC: {"npcs":[{"operation":"update","targetId":"<точный npcId>","npc":{"resourceDeltas":{"health":-3},"statDeltas":{"strength":-1},"upsertStatusEffects":[],"abilityChanges":[{"abilityId":"<точный abilityId>","masteryDelta":2,"history":{"title":"...","description":"..."}}],"strategy":{"observedPlayerPatterns":["..."],"currentPlan":"...","contingencies":["..."]}}}}]}. Новые/раскрытые силы NPC добавляй через npc.upsertAbilities с той же полной детализацией, что у героя; удаляй через npc.removeAbilityIds. Не заменяй целиком stats/resources/statusEffects ради одного изменения: используй их deltas/upsert/remove поля.
...
function compactCampaign(campaign: Campaign, input: string) {
  ...
  return {
    ...
    presentNpcs,
    party,
    npcDirectory: campaign.npcs.map(({ id, name, role, status, currentGoal, lastSeen, relationship, relationshipDimensions, initiative, strategy, recruitment, voice, knowledge, stats, resources, statusEffects, abilities }) => ({ id, name, role, status, currentGoal, lastSeen, relationship, relationshipDimensions, initiative, strategy, recruitment, voice, knowledge, stats, resources, statusEffects, abilities })),
...
- Для устойчивых изменений устройства мира используй world: addRules/removeRules, upsertFactions/removeFactions, upsertLocations/removeLocations, addMysteries/resolveMysteries, upsertLaws/removeLawIds, upsertMechanics/removeMechanicIds, upsertInterfaceModules/removeInterfaceModuleIds, calendarDayDelta/calendarLabel. rules — только истины реальности; общественные законы помещай в upsertLaws, а новые стабильные правила игры — в upsertMechanics. Не меняй мир из-за одной красивой фразы. Новые законы, механики и фракции требуют причины, участника, ресурса и последствия; раскол/слияние сохраняет прежнюю фракцию со статусом dormant/dissolved, а не стирает её.
...
Верни только JSON с полями outcome, beats (${lengthGuide}), suggestions (2–4) и statePatch. Допустимые ключи statePatch: inventory, playerProfile, upsertStats, removeStatKeys, upsertResources, removeResourceKeys, statDeltas, resourceDeltas, currencyDeltas, addAbilities, removeAbilityIds, abilityChanges, artifactChanges, addConditions, removeConditions, upsertStatusEffects, removeStatusEffectIds, relationships, npcs, quests, lore, scene, world, socialLinks, threads, worldEvents, factionReputationDeltas, upsertFactionReputation, party, upsertCharacterArcs, upsertMysteryCases, upsertAntagonistPlans, upsertInfluenceAssets, removeInfluenceAssetIds, memories, events. Все отсутствующие изменения можно опустить. outcome — строка; beats и suggestions — массивы строк; statePatch — объект. Не используй null вместо массива или объекта. Любые числовые поля возвращай JSON-числами, любые флаги — true/false. Текст — на русском.`,
      },
      {
        role: 'user' as const,
...
- quests: принятые, продвинутые, выполненные, проваленные задачи, обещания, долги и сюжетные нити;
...
- world: фракции, репутация, маршруты, правила, места, тайны, фоновые и отложенные события;
...
export function memoryCuratorPrompt(campaign: Campaign, input: string, narrative: string, plan: unknown) {
  ...
  return [
    {
      role: 'system' as const,
      content: `Ты — архивариус очень долгой ролевой кампании. Из завершившегося хода выдели только устойчивые факты, обещания, отношения, тайны и последствия, которые понадобятся через десятки или тысячи ходов. Не дублируй очевидное. Создай краткий архив сцены раз в 4 хода; на каждом 16-м ходу дополнительно архив главы. В archives указывай точный диапазон ходов, теги, реальные entityIds и важность. Верни только JSON {"memories":[{"kind":"fact","content":"...","tags":["..."],"importance":80}],"archives":[{"kind":"scene","title":"...","summary":"...","startTurn":0,"endTurn":4,"tags":["..."],"entityIds":["реальный id"],"importance":80}]}. kind памяти — summary|fact|promise|relationship|mystery; kind архива — scene|chapter|era. importance и номера ходов — числа, tags/entityIds — массивы. Если сохранять нечего, верни пустые массивы.`,
    },
...
- Для каждой фракции опиши не только отношение к герою, но реальную силу 0–100, сферу влияния, территорию, доступные ресурсы, несколько целей, текущий самостоятельный ход, публичный образ, происхождение и секреты. Фракции должны иметь пересекающиеся интересы и материальные возможности действовать.
...
Допустимые resource.kind: health, stamina, mana, energy, focus, sanity, morale, hunger, ammo, charges, custom. Допустимые item.state: intact, damaged, broken, depleted, sealed. Допустимые ability.kind: active, passive, reaction, ritual, transformation, other. Допустимые category сил: ${powerCategoryValues}. Допустимые canonStatus: canonical, derived, original. Допустимые component.status: active, dormant, missing, damaged, destroyed. Допустимые characterArcs.status: active, completed, broken. Допустимые mysteryCases.status на старте не указывай — приложение установит open. Допустимые antagonistPlans.status: active, completed, failed, abandoned; steps.status: pending, active, completed, failed, abandoned. Допустимые influenceAssets.kind: favor, debt, leverage, contact, access, reputation, oath, other; status: active, spent, repaid, lost. Допустимые faction.status: active, dormant, dissolved. Допустимые law.status: proposed, active, contested, repealed; law.visibility: known, rumored, hidden. Допустимые mechanic.category: power, social, economic, travel, crafting, survival, political, other; mechanic.status: emerging, active, obsolete. Допустимые interfaceModules.placement: scene, hero, inventory, world; visual: meters, nodes, slots, track, ledger, signals, radar; icon: spark, eye, shield, network, pulse, compass, crown, rune, gear, flame, star, moon; element.kind: meter, value, badge, node, slot, step, text; element.state: normal, positive, warning, danger, locked, inactive; binding.domain: custom, player.resource, player.stat, player.currency, player.condition-count, scene.tension, world.day, faction.reputation, inventory.category-count, inventory.item-charges, quest.active-count, npc.resource, npc.relationship. initiative.visibility и strategy.visibility: known, rumored, hidden. Все mastery, attunement, urgency, progress, pressure, power, priority, intelligence, tacticalSkill, strategicSkill, predictionSkill, adaptability, deceptionSkill и riskTolerance — числа от 0 до 100; bond и грани отношений — числа от -100 до 100. costs всегда массив объектов, даже когда пуст.
...
12. Живой мир готов к самостоятельному развитию: каждая фракция имеет конкретные goals/currentMove/resources/territory/power; laws отделены от метафизических rules и содержат власть, область и последствия; mechanics имеют причинный source, проверяемый trigger и игровые effects. Они связаны с конфликтом, локациями и будущими worldEvents, а не заполнены универсальными фразами.
...
Через statePatch можно редактировать героя, характеристики и ресурсы, способности, эффекты, предметы и артефакты, NPC и их способности/знания/стратегии/готовность к отряду, связи, задания, лор, сцену, события, фракции, маршруты, тайны, законы, механики, адаптивные интерфейсные модули, память, планы и прочее постоянное состояние. Профиль мира поддерживает world.name/tagline/inspiration/genre/tone/overview/era/system/presentation. Для world.system можно менять name, summary, progression, conflictResolution, consequences, equipmentSlots. Для world.presentation — цвета HEX, surface, motif и подписи интерфейса. Политические законы меняй через world.upsertLaws/removeLawIds, устойчивые правила игры — через world.upsertMechanics/removeMechanicIds, фракции — через полные причинные upsertFactions, модули — через world.upsertInterfaceModules/removeInterfaceModuleIds. Сохраняй прежний id изменяемой сущности.
