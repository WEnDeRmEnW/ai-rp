...
describe('world architect prompt', () => {
  it('requires complete, model-authored NPC records', () => {
    ...
    expect(system.content).toContain('abilities[{name,description,rank,source,cooldown?,kind,mastery,costs?[] (только существующие числовые затраты)')
    ...
    expect(system.content).toContain('capabilities[],synergies?[],counters?[],examples[],techniques[{name,description,kind,category,mastery,activation,scale,costs?[] (только существующие числовые затраты),effects[],requirements?[],limitations?[],unlocked}],canonStatus')
