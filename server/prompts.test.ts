...
describe('world architect prompt', () => {
  it('requires complete, model-authored NPC records', () => {
    ...
    expect(system.content).toContain('abilities[{name,description,rank,source,cooldown?,kind,mastery,costs?[{resource,amount}]')
    ...
    expect(system.content).toContain('capabilities[],synergies?[],counters?[],examples[],techniques[{name,description,kind,category,mastery,activation,scale,costs?[{resource,amount}]')
