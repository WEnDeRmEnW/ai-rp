...
function transcendentPlan(worldImpact = 98) {
  const power = (id: string, name: string) => ({
    ...
    examples: [`Use ${name} to make an otherwise impossible consequence follow from an established cause.`, `Use ${name} to preserve a chosen consequence while its ordinary cause is removed.`],
    techniques: [],
    category: 'reality' as const,
    scale: 'cosmic',
    canonStatus: 'original' as const,
  })
      ...
      inventory: [{
        ...
        item: {
          ...
          artifact: {
            ...
            creativeIdentity: {
              coreFantasy: 'The bearer edits the grammar connecting causes to consequences.',
              centralConcept: 'A crown that treats causality as a writable legal relation rather than as energy.',
              physicalForm: 'Seven disconnected black-gold arcs orbit the bearer without touching the head.',
              originPattern: 'Forged in the master workshop from the uncommitted interval before the first consequence.',
              interactionModel: 'The bearer names two real events and declares the exact relation that shall bind or separate them.',
              signatureExperience: 'The world pauses at the declared relation and then resumes with a different causal grammar.',
              conceptualDomains: ['causality', 'declaration', 'logical consistency'],
              mechanicVerbs: ['declare', 'sever', 'bind', 'preserve'],
              motifs: ['seven open arcs', 'unwritten verdicts'],
              differentiation: ['It governs relations between events rather than time or probability.', 'Its shape never closes because a closed crown would become subject to local causality.'],
            },
            presentation: {
              layout: 'constellation', motif: 'Seven unwritten verdicts', symbol: '⟐', accent: '#d9b45f', secondary: '#7f8cff',
              surface: 'void', glow: 'halo', headerStyle: 'ceremonial', density: 'cinematic',
              sectionOrder: ['identity', 'principle', 'powers', 'combined', 'requirements', 'failureModes', 'origin', 'history'],
              summary: 'Seven open arcs wait for a relation the bearer is prepared to define exactly.',
            },
            discovery: {
              awareness: 100,
              revealedSections: ['identity', 'origin', 'principle', 'requirements', 'passives', 'powers', 'combined', 'failureModes'],
              powerKnowledge: { 'power-declare': 'understood', 'power-sever': 'understood', 'power-bind': 'understood' },
              componentKnowledge: {}, evidence: [], updatedTurn: 0,
            },
